...
def plot_deltas_heatrate(
    ...
    *,
    ...
    for t, ax in zip(ts, axes):
        ...
        if spottype == 'power':
            test_spot_power = np.linspace(model.SP0 - 10, model.SP0 + 10, n_spots).astype(np.float32)
            test_spot_gas = np.linspace(model.SG0, model.SG0, n_spots).astype(np.float32)
        else:
            test_spot_power = np.linspace(model.SP0, model.SP0, n_spots).astype(np.float32)
            test_spot_gas = np.linspace(model.SG0 - 10, model.SG0 + 10, n_spots).astype(np.float32)

        test_delta = compute_nn_delta(model, t, test_spot_power, test_spot_gas, deltatype)
        est_delta = compute_bs_delta(model, t, test_spot_power, test_spot_gas, deltatype)
